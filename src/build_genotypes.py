...
def filter_multiallelic_and_monomorphic(
    ...
    """Filter multiallelic and monomorphic sites only. No MAF filter."""
    ...
    polymorphic_mask   = hap.var(axis=1) > 0
...
def apply_maf_filter_on_discovery(
    ...
    maf_report = {
        ...
        "pct_causal_lost":         100 * len(causal_lost) / len(causal_before) if causal_before else 0,
    }

    print(f"  sites after MAF filter (CEU)   : {int(maf_mask.sum())}")
    print(f"  causal sites lost to MAF       : {maf_report['causal_sites_lost']}/{maf_report['causal_sites_before_maf']} ({maf_report['pct_causal_lost']:.1f}%)")
...
def simulate_trait(
    ...
    causal_site_ids = rng.choice(biallelic_site_ids, size=num_causal, replace=False).tolist()
    model    = tstrait.trait_model(distribution=distribution, mean=0, var=1)
    trait_df = tstrait.sim_trait(ts, causal_sites=causal_site_ids, model=model,
                                 alpha=0, random_seed=42)
...
def simulate_phenotype(
    ...
    model        = tstrait.trait_model(distribution=distribution, mean=0, var=1)
    phenotype_df = tstrait.sim_phenotype(ts, model=model, causal_sites=causal_site_ids,
                                         alpha=alpha, h2=h2, random_seed=random_seed)
