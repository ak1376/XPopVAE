...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Grid index (sim_number wildcard from Snakemake)",
    )
    ap.add_argument("--replicate", type=int, default=0, help="Replicate index")
...
def main() -> None:
    ...
    maf_threshold = (
        args.maf_override
        if args.maf_override is not None
        else float(cfg.get("maf_threshold", 0.0))
    )
    ...
    # AFTER
    split_rng = np.random.default_rng(seed)  # constant for all sims/reps
    trait_rng = np.random.default_rng(
        seed + args.sim_number * 1000
    )  # varies per sim_number only
    ...
    for ind_id, pop in ind_id_to_pop.items():
        if pop == discovery_pop:
            (
                train_discovery_inds
                if split_rng.random() < disc_train_frac
                else validation_discovery_inds
            ).append(ind_id)
        else:
            (
                test_target_inds
                if split_rng.random() < target_held_out_frac
                else train_target_inds
            ).append(ind_id)
    ...
    (
        train_discovery_pheno,
        validation_discovery_pheno,
        train_target_pheno,
        test_target_pheno,
    ) = simulate_phenotype(
        ts,
    ...
    validation_discovery_diploid = diploid[validation_discovery_inds, :].astype(
        np.float32
    )
    ...
    def pheno_values(df: pd.DataFrame) -> np.ndarray:
        return (
            df.sort_values("individual_id")["phenotype"].to_numpy().astype(np.float32)
        )
    ...
    np.save(
        pheno_dir / "discovery_train_pheno.npy", pheno_values(train_discovery_pheno)
    )
    np.save(
        pheno_dir / "discovery_validation_pheno.npy",
        pheno_values(validation_discovery_pheno),
    )
    ...
    ind_id_to_pop_name = {
        ind.id: pop_id_to_name[ind.population] for ind in ts.individuals()
    }
    ...
    np.save(
        outdir / "discovery_val_idx.npy", validation_discovery_inds.astype(np.int64)
    )
